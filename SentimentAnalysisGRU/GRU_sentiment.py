...
model_filename = dataset_category + "model.pt"
...
######################################################
### PERFORM TRAINING - SAVE MODEL OF HIGHEST ACCURACY
######################################################

for i in range(epochs):
    rnn.train(train_iter)
    accuracy = rnn.predict(test_iter)
    print('Accuracy of the network in epoch {}: {}'.format(i, accuracy))
    if i==0:
...
rnn = torch.load('./models/sentimentmodel74.pt')
accuracy = rnn.predict(test_iter)
